...
int areAllFalse(bool array[], int size, int NMF);
...
//   #pragma omp parallel for firstprivate(rand) 
        ...
        //x += t * dir_x;
        #pragma omp parallel for shared(x, y, z) num_threads(6)
        for (int k=0; k < 8 ; ++k) {
...
int main(void) {
   ...
   // simulation
   //#pragma omp parallel //for num_threads(8) firstprivate(rand)
